...
int* lista_pausas;
int estado = 0; // painel de controle  
int segmento;  
...
void handler_SIGINT(int sig) {
    // Ctrl-C: liga o round-robin
    if (estado == 0) {
        estado = 1;
        printf("\n[PAI] Recebi Ctrl-C → iniciando/retomando round robin\n");
    }
}

void handler_SIGTSTP(int sig) {
    // Ctrl-Z: pausa o round-robin
    if (estado == 1) {
        estado = 0;
        printf("\n[PAI] Recebi Ctrl-Z → pausando round robin\n");
    }
}

void handler_SIGQUIT(int sig) {
    // Ctrl-\: encerra tudo
    printf("\n[PAI] Recebi Ctrl-\\ → encerrando filhos e pai\n");
...
int main(void){
    ...
    printf("[PAI] Handler SIGINT instalado\n");

    // numero de processos filhos
    printf("Quantas aeronaves: \n");
    scanf("%d", &qtd_aeronaves);
    printf("Acabou o scanf\n");
    /*
    printf("Aguardando Ctrl-C para começar...\n");
    pause();  // vai “dormir” aqui até chegar SIGINT, SIGTSTP ou SIGQUIT
    */
    ...
    // round robin
    while(cont-- != 0){
        //if (estado == 1){ // Só executa o round-robin se estiver em "execução"
            for(i = 0; i < qtd_aeronaves; i++){
                if (estado == 1){ // Só executa o round-robin se estiver em "execução"
                    ...
                    // estado == 0: estamos pausados
                    printf("[PAI] Pausado: aguardando Ctrl-C ou Ctrl-\\\n");
                    pause();  // volta aqui quando Ctrl-C (retoma) ou Ctrl-\\ (mata)
                }       
        }
    }
    for(i = 0; i < qtd_aeronaves; i++){
